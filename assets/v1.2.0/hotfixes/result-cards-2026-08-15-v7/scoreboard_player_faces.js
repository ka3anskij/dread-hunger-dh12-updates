...
(function () {
    ...
    var profileByRoleOwner = Object.create(null);
    var profileOrder = [];
    ...
    function rememberProfile(playerName, icon, roleLabel, roleData) {
        ...
        if (
            ...
            profileByRoleOwner[roleData.toString()] = profile;
        }
        return profile;
    }
    ...
    Interceptor.attach(playerStateSetPlayerRole, {
        onEnter: function (args) {
            try {
                ...
                var playerName = readFString(playerState.add(0x300));
                var incomingObjectName = getObjectName(roleData);
                var existingProfile = profileByPlayerName[
                    playerNameKey(playerName)
                ];

                // The replay/results path creates a fresh PlayerState and
                // assigns PR_OldSailor to every custom role.  Restore the
                // genuine role object captured during the live match.  A
                // real old sailor keeps PR_OldSailor and is left untouched.
                if (
                    incomingObjectName.indexOf('PR_OldSailor') !== -1 &&
                    existingProfile !== undefined &&
                    existingProfile.roleData !== null &&
                    !existingProfile.roleData.isNull()
                ) {
                    var savedObjectName = getObjectName(
                        existingProfile.roleData
                    );
                    if (
                        savedObjectName.indexOf('PR_OldSailor') === -1 &&
                        roleLabelFromData(existingProfile.roleData).length !== 0
                    ) {
                        debug(
                            'replace result fallback role player=' +
                            playerName + ' old=' + incomingObjectName +
                            ' new=' + savedObjectName
                        );
                        roleData = existingProfile.roleData;
                        args[1] = roleData;
                    }
                }

                var profile = rememberProfile(
                    playerName,
                    null,
                ...
                if (profile !== null) {
                    profileByRoleOwner[playerState.toString()] = profile;
                    profileByRoleOwner[roleData.toString()] = profile;
                }
                debug(
    ...
    // The result screen resolves a new PlayerState for every replay slot.
    // Associate that transient object with the pregame profile so every
    // subsequent role-icon lookup can use the correct face portrait.
    Interceptor.attach(base.add(0xEA91D0), {
        onEnter: function (args) {
            try {
                this.playerIndex = args[0].toUInt32() & 0xFF;
            } catch (_) {
                this.playerIndex = -1;
            }
        },
        onLeave: function (retval) {
            try {
                if (
                    this.playerIndex < 0 ||
                    this.playerIndex >= profileOrder.length ||
                    retval.isNull()
                ) {
                    return;
                }
                var profile = profileOrder[this.playerIndex];
                if (profile === null || profile === undefined) {
                    return;
                }
                profileByRoleOwner[retval.toString()] = profile;
                if (
                    profile.roleData !== null &&
                    !profile.roleData.isNull()
                ) {
                    profileByRoleOwner[profile.roleData.toString()] = profile;
                }
                debug(
                    'resolveScoreboardPlayer index=' + this.playerIndex +
                    ' owner=' + retval + ' player=' + profile.name +
    ...
    Interceptor.attach(getRoleIconAddress, {
        onEnter: function (args) {
            ...
            try {
                var owner = args[0];
                if (!owner.isNull()) {
                    this.profile = profileByRoleOwner[owner.toString()];
                }
                if (
                    this.profile !== null &&
                    ...
                    debug(
                        'mapped getRoleIcon owner=' + owner +
