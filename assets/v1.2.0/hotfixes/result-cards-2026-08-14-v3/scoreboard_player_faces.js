...
(function () {
    ...
    function rememberPregameProfile(playerName, icon) {
        var now = Date.now();
        ...
        var profile = rememberProfile(playerName, icon, '');
        if (profile === null) {
            return;
        }

        var key = playerNameKey(profile.name);
        for (var index = 0; index < profileOrder.length; index++) {
            if (playerNameKey(profileOrder[index].name) === key) {
                profileOrder[index] = profile;
                return;
            }
        }
    ...
    function resolveResultProfile(capture) {
        ...
        try {
            var replayPlayer = resolveScoreboardPlayer(
                capture.record.add(0x2E).readU8(),
                capture.item
            );
            if (!replayPlayer.isNull()) {
                var playerName = readFString(replayPlayer.add(0x30));
                profile = profileByPlayerName[playerNameKey(playerName)] || null;
            }
        } catch (_) {}

        if (profile === null && capture.slot < profileOrder.length) {
            profile = profileOrder[capture.slot];
        }
